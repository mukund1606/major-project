...
class Node:
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if self.type != NodeType.HIDDEN:
            text = Node.FONT.render(self.label, 1, Color.DARK_BLUE)
            screen.blit(
                text,
                (
                    self.x
                    + (self.type - 1)  # type: ignore
    ...
    def get_color(self):
        ...
        for i in range(3):
            color[0][i] = int(
                ratio * (self.colors[1][i] - self.colors[3][i]) + self.colors[3][i]
            )
            color[1][i] = int(
                ratio * (self.colors[0][i] - self.colors[2][i]) + self.colors[2][i]
            )
        return color
