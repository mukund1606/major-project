...
class SimulationUI:
    def __init__(self, root):
        ...
        # Create frame for top controls
        self.controls_frame = ttk.Frame(self.root)
        self.controls_frame.pack(fill="x", padx=5, pady=5)

        # Add live view toggle button
        self.is_live = tk.BooleanVar(value=IS_LIVE)
        self.live_button = ttk.Checkbutton(
            self.controls_frame,
            text="Live View",
            variable=self.is_live,
        ...
        self.finish_line_stats_tab = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.finish_line_stats_tab, text="Finish Line Stats")
        ...
        self.fitness_progression_figure, self.fitness_progression_ax = plt.subplots(
            figsize=(5, 4)
        )
        self.fitness_distribution_figure, self.fitness_distribution_ax = plt.subplots(
            figsize=(5, 4)
        )
        ...
        self.finish_line_figure, self.finish_line_ax = plt.subplots(figsize=(5, 4))

        # Create canvas for each tab
        self.sensor_canvas = FigureCanvasTkAgg(
            self.sensor_figure, master=self.sensor_tab
        )
        self.sensor_canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)

        self.avg_fitness_canvas = FigureCanvasTkAgg(
            self.avg_fitness_figure, master=self.avg_fitness_tab
        )
        self.avg_fitness_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )

        self.ind_fitness_canvas = FigureCanvasTkAgg(
            self.ind_fitness_figure, master=self.ind_fitness_tab
        )
        self.ind_fitness_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )
        ...
        self.fitness_progression_canvas = FigureCanvasTkAgg(
            self.fitness_progression_figure, master=self.fitness_progression_tab
        )
        self.fitness_progression_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )

        self.fitness_distribution_canvas = FigureCanvasTkAgg(
            self.fitness_distribution_figure, master=self.fitness_distribution_tab
        )
        self.fitness_distribution_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )

        self.dynamics_canvas = FigureCanvasTkAgg(
            self.dynamics_figure, master=self.dynamics_tab
        )
        self.dynamics_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )

        self.crash_stats_canvas = FigureCanvasTkAgg(
            self.crash_stats_figure, master=self.crash_stats_tab
        )
        self.crash_stats_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )

        self.finish_line_canvas = FigureCanvasTkAgg(
            self.finish_line_figure, master=self.finish_line_stats_tab
        )
        self.finish_line_canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True
        )
    ...
    def init_plots(self):
        ...
        self.finish_line_ax.set_title("Finish Line Statastics")
        self.finish_line_ax.set_xlabel("Generation")
        self.finish_line_ax.set_ylabel("Cars Reached Finish Line")

    def toggle_live_view(self):
        global IS_LIVE
        IS_LIVE = self.is_live.get()
        print(f"Live view: {'ON' if IS_LIVE else 'OFF'}")
        self.update_plots()

    def on_tab_change(self, event):
        # Reset view to normal when tab changes
        for fig in [
            self.sensor_figure,
            self.avg_fitness_figure,
            self.ind_fitness_figure,
            self.path_figure,
            self.fitness_progression_figure,
            self.fitness_distribution_figure,
            self.dynamics_figure,
            self.crash_stats_figure,
            self.finish_line_figure,
        ]:
    ...
    def read_data_file(self):
        try:
            file_path = LIVE_DATA_FILE if IS_LIVE else DATA_FILE
    ...
    def update_plots(self):
        ...
        if self.ind_fitness:
            self.fitness_progression_ax.plot(
                range(len(self.ind_fitness)), self.ind_fitness, "r-", label="Max"
            )
        if self.avg_fitness:
            self.fitness_progression_ax.plot(
                range(len(self.avg_fitness)), self.avg_fitness, "b--", label="Avg"
            )
        self.fitness_progression_ax.set_title("Fitness Progression")
        ...
        if self.ind_fitness:
            ...
            if bins > 0:
                self.fitness_distribution_ax.hist(
                    self.ind_fitness, bins=bins, color="skyblue", edgecolor="black"
                )
        self.fitness_distribution_ax.set_title(
            f"Fitness Distribution - Gen {self.generation}"
        )
        ...
        if velocities:
            self.dynamics_ax.plot(
                range(len(velocities)), velocities, "g-", label="Velocity"
            )
        if headings:
        ...
        if crash_history:
            self.crash_stats_ax.bar(
                range(len(crash_history)), crash_history, color="red"
            )
        self.crash_stats_ax.set_title("Crashes per Generation")
        self.crash_stats_canvas.draw()

        # Update finish line stats plot
        self.finish_line_ax.clear()
        finish_line_history = data.get("finish_line_history", [])
        if finish_line_history:
            self.finish_line_ax.bar(
                range(len(finish_line_history)), finish_line_history, color="green"
            )
        self.finish_line_ax.set_title("Finish Line Statastics per Generation")
        self.finish_line_canvas.draw()
